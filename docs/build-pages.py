...
function audCells(o, note) {
  ...
  return `<td>${orderLink(o.orderName, o.orderId)}${note || ''}</td>
    <td class="cust-cell">${customerCell(o)}</td>
...
#    «السجل ناقص».
CL_READY   = r'''          <li class="cl-item"><span class="cl-tag change">تعديل</span><span>🔴 <b>الطابور بقى بيبدأ من أوردرات <code>01/04/2026</code></b> — الـ Worker <b>مابيجيبش</b> اللي أقدم من كده خالص (أوردرات الدورة القديمة العالقة كانت بتملا الطابور بمئات الصفوف بادجها «متأخر ١٧١ يوم»). ⚠️ <b>والأرضية مكتوبة جنب الرقم فوق الطابور</b> وجاية من رد الـ Worker نفسه — استبعاد صامت من رقم كان هيخلّي الفرق بينه وبين الواقع بلا تفسير. ⛔ ومحتاج <code>ready-orders-worker</code> <b>1.4.0</b>.</span></li>
          <li class="cl-item"><span class="cl-tag change">تعديل</span><span>🔴 <b>عمود «العميل» بقى سطرين بالكتير</b> — بنط أصغر وحشو أضيق، والاسم الطويل بيتقص بـ<code>…</code>. الاسم الخماسي كان بيلفّ على <b>ستة سطور</b> ويخلّي الصف ضعف ارتفاع جاره. ⚠️ <b>والاسم الكامل مش ضايع</b>: قف على الخلية يبان كامل، والبحث بيدوّر فيه كامل. ⛔ <b>وبلا أي توسيع للعمود</b> — العرض الزيادة بتاع «العنوان» و«ملحوظات» ما اتلمسش.</span></li>
'''

CL_SHIPPED = r'''          <li class="cl-item"><span class="cl-tag change">تعديل</span><span>🔴 <b>الطابور بقى بيبدأ من أوردرات <code>01/04/2026</code></b> — الـ Worker <b>مابيجيبش</b> اللي أقدم من كده خالص. ⚠️ <b>وأثرها هنا أكبر</b>: ده الطابور اللي <b>بيتراكم</b> بطبيعته، فالصف اللي «نسي حد يقفله» من قبل التاريخ ده <b>مابقاش بيبان هنا خالص</b> — ومكانه بقى استعلام على شوبيفاي مباشرةً. والأرضية مكتوبة <b>جنب الرقم فوق الطابور</b> وجاية من رد الـ Worker. ⛔ ومحتاج <code>shipped-orders-worker</code> <b>1.1.0</b>.</span></li>
          <li class="cl-item"><span class="cl-tag change">تعديل</span><span>🔴 <b>عمود «العميل» بقى سطرين بالكتير</b> — بنط أصغر وحشو أضيق، والاسم الطويل بيتقص بـ<code>…</code>، والاسم الكامل بيبان بالوقوف على الخلية والبحث بيدوّر فيه كامل. ⛔ <b>وبلا أي توسيع للعمود</b>.</span></li>
'''
...
/* 🔴 أرضية التاريخ جنب الرقم — **مش بشكل مربع عدّ** عن قصد: المربعات
   حواليها بتعدّ أوردرات، ودي بتقول **نطاق**. شكل واحد كان هيخلّي
   «01/04/2026» تتقري رقم تالت في صف أرقام. */
.cb-scope { font-size: 11.5px; font-weight: 800; color: var(--text-secondary); background: var(--surface-2); border: 1px solid var(--border); border-radius: 20px; padding: 2px 9px; white-space: nowrap; }
.cb-scope[hidden] { display: none; }
...
/* 🔴 **خلية العميل — سطران بالكتير** (v1.9.0 · طلب أحمد 17-09-2026).
   العمود ٩٠px بقرار، والاسم الخماسي كان بيلفّ على ستة سطور ويطوّل الصف
   كله — والتوسيع **مرفوض صراحةً** (الجدول ماعندوش عرض زيادة يتاخد من
   «العنوان» و«ملحوظات»، وهما العمودان اللي بيتقروا فعلاً).
   ⚠️ **والتلاتة لازمين مع بعض** زي خلية التاريخ بالظبط:
     ① حشو أضيق (6px بدل 11) → ٧٨px محتوى بدل ٦٨.
     ② بنط أصغر بدرجتين (`12.5px`) → كلمتين في السطر بدل كلمة.
     ③ وقص عند سطرين بـ`…` — من غيره التصغير بيقلّل السطور مش بيسقّفها،
        والاسم الطويل جدًا يفضل بيطوّل الصف.
   ⛔ **`-webkit-box` على `<span>` جوّاني مش على `<td>`** — على خلية جدول
      بيشيلها من تخطيط الجدول والصف بيتفكك بالكامل.
   ⚠️ **و`overflow-wrap:anywhere`** — اسم من غير مسافات أطول من العمود كان
      بيخرج من حدوده ويزقّ باقي الأعمدة.
   ⚠️ **والاسم الكامل في `title`** (والبحث والتصدير بيشوفوه كامل) — قص بلا
      منفذ للقيمة الكاملة كان هيخلّي الموظف يفتح الأوردر عشان يقرا اسم. */
.data-table td.cust-cell { padding-inline: 6px; }
.cust-txt { display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2; line-clamp: 2; overflow: hidden; font-size: 12.5px; line-height: 1.4; overflow-wrap: anywhere; cursor: help; }
...
__AUDIT_TABS__
  ...
  <div class="card count-box">
    ...
    <div style="display:flex;align-items:baseline;gap:8px">
      ...
      <!-- 🔴 **أرضية تاريخ الطابور — مكتوبة جنب الرقم نفسه** (v1.9.0 ·
           طلب أحمد 17-09-2026). الـ Worker بقى **مابيجيبش** أي أوردر أقدم
           من التاريخ ده، والرقم الكبير جنبها بيعدّ المجاب بس — فاستبعاد
           صامت هنا معناه رقم أقل من الواقع **بلا أي تفسير على الشاشة**.
           🔴 **والقيمة جاية من رد الـ Worker** (`minCreatedDay`) مش مكتوبة
              في الصفحة: التاريخ في مكانين بيتعدّلوا بالإيد = درس R1
              حرفيًا — الـ Worker يترفع والشاشة تفضل بتقول التاريخ القديم.
           ⚠️ **وبتختفي خالص لو الرد مافيهوش الحقل** (Worker أقدم من
              `ready 1.4.0` / `shipped 1.1.0`) — ادعاء أرضية مش مطبّقة
              أسوأ من مفيش ادعاء. -->
      <span class="cb-scope" id="qScope" hidden></span>
    </div>
    <div class="cb-mid">
    ...
    <div class="eco-modal-body">
      <details class="about-sec" open>
        ...
        <div class="about-sec-body">
          ...
          <p>🔴 <b>والطابور ليه أرضية تاريخ</b> (طلب أحمد 17-09-2026): الـ Worker
             <b>مابيجيبش</b> أي أوردر أقدم من التاريخ المكتوب <b>جنب الرقم فوق
             الطابور</b> — أوردرات الدورة القديمة العالقة كانت بتملا الطابور
             بمئات الصفوف بادجها «متأخر ١٧١ يوم» وبتدفن شغل اليوم.
             ⚠️ <b>والاستبعاد ده مُعلَن مش صامت</b>: التاريخ على الشاشة، وجاي من
             <b>رد الـ Worker نفسه</b> (مش مكتوب في الصفحة) فمستحيل الاتنين
             يفترقوا. ⛔ والأوردر الأقدم من كده <b>مش بيتعدّ في الرقم الكبير
             ولا في «مستحق التحصيل»</b> — مكانه بقى استعلام على شوبيفاي
             مباشرةً.</p>
          <h4>الـ Workers</h4>
          ...
          <ul>
            ...
            <li><b>العميل</b> — اسم المستلم من عنوان الشحن. ⚠️ العمود ضيق بقرار
                (العرض الزيادة راح للأعمدة اللي بتتقرا فعلاً)، فالاسم بيتكتب
                <b>بخط أصغر وبيتقص عند سطرين بـ…</b>. 🔴 <b>والاسم الكامل
                مش ضايع</b>: قف على الخلية يبان كامل، و<b>البحث بيدوّر في
                الاسم كامل</b> مش في المقصوص.</li>
__ABOUT_COLS__
...
const qState = { rows: [], at: null, loading: false, dirty: false, timer: null,
                 ...
                 // 🔴 أرضية تاريخ الطابور **زي ما الـ Worker قالها** — `null`
                 //    معناها «الـ Worker ما قالش»، وساعتها الشاشة مابتدّعيش
                 //    أي أرضية (Worker أقدم من تسليم الأرضية).
                 minDay: null };
...
// 🔴 **عمود «العميل» — سطران بالكتير** (v1.9.0 · طلب أحمد 17-09-2026).
//    العمود عرضه ٩٠px بقرار (تمريرة v1.7.0)، والاسم الرباعي/الخماسي كان
//    بيلفّ على **ستة سطور** ويطوّل الصف كله: صف واحد كان بياخد ضعف ارتفاع
//    جاره، والجدول بيتقري «مكسور» مش «فيه اسم طويل».
// 🔴 **الحل تصغير + قص مُعلَن — مش توسيع العمود** (اللي اتطلب صراحةً):
//    ① بنط أصغر (`12.5px` بدل `14.5`) وحشو أضيق → كلمتين في السطر بدل
//       واحدة، فأغلب الأسماء بتخلص في سطر أو سطرين من غير أي قص.
//    ② والباقي بيتقص عند **سطرين بالظبط** بـ`…` ظاهرة.
// 🔴 **والاسم الكامل ما ضاعش** — في `title` على الخلية (بيبان بالوقوف
//    عليها)، وفي **البحث** (بيدوّر في الاسم كامل مش في المعروض)، وفي
//    **تصدير الجرد**. قص بلا منفذ للقيمة الكاملة كان هيخلّي الموظف يفتح
//    الأوردر على شوبيفاي عشان يقرا اسم.
// ⛔ **والقص على العنصر الجوّاني مش على `<td>`** — `display:-webkit-box`
//    على خلية جدول بيشيلها من تخطيط الجدول بالكامل والصف بيتفكك.
// ⚠️ **و`—` للاسم الفاضي** زي باقي الأعمدة — خانة فاضية بتتقري «الشاشة
//    بايظة».
function customerCell(o) {
  const t = (o.customer || '').trim();
  return t ? `<span class="cust-txt" title="${esc(t)}">${esc(t)}</span>`
...
function qRender() {
  ...
  // 🔴 **أرضية التاريخ** — بتتكتب بس لما الـ Worker يقولها، وبصيغة اليوم
  //    اللي الجدول بيعرض بيها التواريخ بالظبط (`dd/mm/yyyy`): صيغتان
  //    لنفس التاريخ على نفس الشاشة بيتقروا تاريخين.
  const scEl = document.getElementById('qScope');
  if (scEl) {
    const d = qState.minDay ? String(qState.minDay).split('-') : null;
    scEl.hidden = !(d && d.length === 3);
    if (!scEl.hidden) scEl.textContent = `الطابور من ${d[2]}/${d[1]}/${d[0]}`;
  }
...
async function qLoad(silent) {
  ...
  try {
    ...
    // ⚠️ **من الرد مباشرةً** — والكاش بيتخزّن خام فالقيمة بترجع معاه.
    qState.minDay    = data.minCreatedDay || null;
...
(function drawFromCache() {
  ...
  qState.minDay    = c.data.minCreatedDay || null;
  qRender();
  qSetFresh();
})();
