...
const DCO_WORKERS = {
  ...
  // 🔴 **`1.4.0` من v1.9.0 — أرضية تاريخ الطابور (`01/04/2026`).**
  //    الـ Worker من `1.4.0` **مابيجيبش** أوردر أقدم من التاريخ ده، وبيرجّع
  //    الأرضية في الرد (`minCreatedDay`) عشان الشاشة تكتبها فوق الرقم.
  //    ⛔ **وعلى Worker أقدم الطابور بيرجع بكامل الأوردرات القديمة** —
  //       يعني القرار اللي أحمد طلبه **مش مطبّق أصلاً**، والشاشة مافيهاش
  //       أي حاجة تقول كده غير غياب شيب الأرضية (وغياب حاجة مابيتلاحظش).
  //       فالحارس هنا هو اللي بيسمّي الأداة والنسخة بدل ما الموظف يشوف
  //       طابور ضعف حجمه ويفتكره الصح.
  //    ⚠️ **والتدهور محصور**: الطابور والجرد وكل الأعمدة شغّالين بالكامل —
  //       اللي بيقع هو **نطاق** الطابور بس.
  ready:   { url: 'https://ready-orders-worker.ecommoda-dev.workers.dev',   min: '1.4.0', label: 'قسم الجاهز للشحن' },
  // 🔴 **`1.1.0` من v1.9.0 — نفس الأرضية بالحرف** (الشرح فوق).
  //    ⚠️ وأثرها هنا أكبر: الطابور ده **بيتراكم** بطبيعته، فالفرق بين
  //       Worker قديم وجديد مئات الصفوف مش عشرات.
  shipped: { url: 'https://shipped-orders-worker.ecommoda-dev.workers.dev', min: '1.1.0', label: 'طابور المشحون' },
  ...
  // 🔴 **رفع أوردرات بوسطة (v1.13.0) — رابع أداة مدموجة، وأول واحدة من
  //    ريبو كان أداة مستقلة كاملة (تلات أوضاع: شحن · استرجاع · استبدال) لا
  //    تابعة أصلًا لهب المخزن ولا نصفها موجود هنا قبل كده.** نفس نمط دمج
  //    `Order-Status-Updater.html`/`COD-Payment-Center.html` (v1.5.0) —
  //    **مش** نمط «التسليم الجزئي» (v1.10.0): الريبو المستقل
  //    (`Bosta-Orders-Upload`) **لسه فيه صفحته وشاشة دخوله ورابطه القديم
  //    شغّالين** — قرار متعمَّد لأن الريبو ده معاه مجموعة اختبارات كبيرة
  //    (١٦ ملف) بتحمّل `index.html` بتاعه مباشرة، ومسحه كان هيسيب
  //    الاختبارات دي بلا ملف تقرا منه. النسختين بينادوا **نفس الـ
  //    Worker**، فمفيش خطر «داتا متفرّقة» — الخطر الوحيد إن **الشكل**
  //    والإصلاحات يفترقوا (نفس درس R1)، ومفيش سكربت مولّد بينهم لسه.
  //    ⚠️ **والـ Worker نفسه ما اتلمسش** — نفس التلات أوضاع ونفس محرك
  //       مطابقة العنوان ونفس السجل.
  //    🔴 **`min = '2.11.0'`** — النسخة اللي فيها `clear_upload_marks`
  //       (مسح التاج/الميتافيلد بعد إلغاء شحنة) و`cancel_re` الشامل للشحن
  //       العادي. Worker أقدم من كده لسه شغّال بمنطق أقدم من الواجهة
  //       المدموجة (زي `fetch_candidates`/`upload_re`/الميتافيلدات الجديدة
  //       من v2.0.0، و`coverageOnly` من v2.2.0، والاسم العربي من v2.8.0) —
  //       الواجهة مبنية عليها كلها فمينفعش الحد الأدنى يبقى أقدم منها.
  //    🔴 **والسر هنا سر الهب** (`delivery_cod_ops`) — نفس شرط التلات
...
function dcoAuditReason(s) {
  ...
  // ④ لسه ما استعلمناش
  // ⚠️ **الحالة دي بقت استثناء مش قاعدة** (v1.4.1): الاستعلام بيشتغل
  //    **تلقائيًا مع «إنهاء الجرد»**، فالصف اللي لسه بيقولها معناه إن
  //    النداء التلقائي **فشل** (Worker أقدم من 1.2.0 أو شبكة واقعة) —
  //    والفعل بقى «إعادة المحاولة» مش «ابدأ الاستعلام».
  // ⛔ والفرق مهم: «اضغط استعلام» على شاشة **اتستعلمت فعلاً** بتخلّي
  //    الموظف يدوّر على زرار، ولو لقاه يضغطه ويستنى نفس الفشل بلا تفسير.
  return { code: 'unknown', label: 'مش في قسم الجاهز للشحن',
    ...
    action: 'الاستعلام بيتم تلقائي مع إنهاء الجرد — لو لسه مش معروفة يبقى النداء فشل، اضغط «إعادة محاولة الاستعلام»' };
}
