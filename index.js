// ══════════════════════════════════════════════════════════════
// §HEADER
// Worker: delivery-cod-operations-center-worker — EcomModa
// Tool:   مركز عمليات الشحن والتحصيل — **الدخول والخروج وبس**
//
// skills: worker-builder v3.7.0 · constants v3.1.0 · html-builder v6.6.0 — 22-09-2026
//
...
//    ريبو واحد). الطابورين أدوات مستقلة في ريبوهاتهم.
//
// ⛔ **وممنوع يتضاف له أي endpoint تشغيلي.** لا طابور ولا سكان ولا تحديث
//    حالة. أي أداة جديدة في المركز بتاخد **Worker بتاعها في ريبوها** (نفس
//    قرار ٨). الاسم بيقول «مركز العمليات» فالإغراء موجود — والقاعدة دي هي
//    اللي بتمنعه: Worker واحد بيخدم أدوات كتير = مصدرين للحقيقة في هب واحد.
//
// 🔴 **بيكتب صفّين بس في D1:** `login` و`logout` تحت
//    `tool = 'delivery_cod_ops_center'`. صفر كتابة على شوبيفاي، وصفر أي
//    قيمة `type` تانية.
//
...
// ══════════════════════════════════════════════════════════════
const TOOL_NAME      = 'delivery_cod_ops_center';
...
//    السر يقدر يكتب صفوف بأي قيمة `tool` — وده حصل فعلاً قبل كده
//    (`write_external_log` في `cod-payment-center-worker` أنتج صفوف يتيمة
//    واتشال في v3.3.0).
...
// الطول لوحده **مش كافي** لإثبات إن أعضاء مجموعة `delivery_cod_ops` شايلين
...
const LOG_REGISTRY = {
  delivery_cod_ops_center: new Set(['login', 'logout']),
};
...
async function handleDiag(env, request) {
  ...
  push(!!fp, 'بصمة WORKER_SECRET', fp
    ? `بصمة ${fp} · مجموعة delivery_cod_ops — لازم تطابق ready-orders-worker و shipped-orders-worker`
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      //    هو نفسه اللي بيدخل بيه محطة المخزن.
      // 🔴 **متساب بقرار أحمد (15-09-2026)** — شيله كان هيمنع الموظف الجديد
      //    من الدخول من المحطة دي خالص، والخطر اللي بيقفله (إنشاء PIN لاسم
      //    لسه مالوش واحد) **موجود بالظبط في هب المخزن**، فشيله من هنا بيقفل
      //    باب من اتنين. مكان الحماية الحقيقي هو الأدمن بانل.
      ...
      // 🔴 **مفيش `get_logs*` — والغياب مقصود.** الهب مالوش تاب سجل، والأداة
      //    بتكتب صفّي دخول/خروج بس. endpoint بلا مستهلك = سطح بلا لازمة (نفس
      //    قرار الطابورين). لو اتطلب عرض سجل الدخول يومًا، مكانه شاشة
      //    الإحصائيات أو الأدمن بانل مش هنا.
